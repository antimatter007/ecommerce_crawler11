# ecommerce_crawler/spiders/bewakoof_spider.py
...
class BewakoofSpider(scrapy.Spider):
    name = 'bewakoof_spider'
    allowed_domains = ['bewakoof.com']
    base_url = 'https://www.bewakoof.com/search/men-mobile-phones'
    count = 1
    ...
    def parse(self, response):
        ...
        product_links = response.css(
            'a.product-potential-click::attr(href)'
        ).getall()
        ...
        for i, link in enumerate(product_links):
            ...
            details['domain'] = 'bewakoof.com'
            details['url'] = link
            details['title'] = titles[i].strip() if i < len(titles) else None
            details['brand'] = 'Bewakoof'  # Static as per domain
